import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { useToast } from "@/components/ui/use-toast";
import { GoogleAuthProvider, signInWithPopup } from "firebase/auth";
import { auth } from "../lib/firebase";
import { fetchGeneralConfig } from "@/services/generalConfigService";

const Index = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const [allowedDomains, setAllowedDomains] = useState<string[]>([]);

  const fetchAllowedDomains = async () => {
    try {
      const config = await fetchGeneralConfig();
      const domains = (config.configEmailDomainAvaliable || "")
        .split(";")
        .map((domain) => domain.trim().toLowerCase());

      setAllowedDomains(domains);
    } catch (error) {
      console.error("Erro ao buscar domínios permitidos:", error.message);
      toast({
        variant: "destructive",
        title: "Erro",
        description: "Não foi possível carregar os domínios permitidos.",
      });
    }
  };

  useEffect(() => {
    fetchAllowedDomains();
  }, []);

  const handleGoogleLogin = async () => {
    try {
      const provider = new GoogleAuthProvider();
      const result = await signInWithPopup(auth, provider);
      const email = result.user.email;

      if (!email) {
        throw new Error("Email não encontrado.");
      }

      // Extrai o domínio do email e converte para minúsculas
      const domain = email.split("@")[1].toLowerCase();

      // Verifica se o domínio está na lista de domínios permitidos
      if (!allowedDomains.includes(domain)) {
        await auth.signOut();
        toast({
          variant: "destructive",
          title: "Acesso Negado",
          description: `Por favor, use um email com um dos seguintes domínios: ${allowedDomains.join(", ")}`,
        });
        return;
      }

      // Salva o email no sessionStorage e redireciona para verificação
      sessionStorage.setItem("userEmail", email);
      navigate("/verify");
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Erro de Autenticação",
        description: "Ocorreu um erro durante o login. Tente novamente.",
      });
    }
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100 p-4">
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-xl shadow-lg">
        <div className="text-center space-y-4">
          <h1 className="text-3xl font-semibold text-gray-900">Bem-vindo</h1>
          <p className="text-gray-600">
            Para acessar o formulário, faça login com sua conta institucional
          </p>
        </div>
        <Button
          onClick={handleGoogleLogin}
          className="w-full flex items-center justify-center space-x-2 py-6 transition-all hover:scale-[1.02]"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="currentColor"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="currentColor"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="currentColor"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          <span>Entrar com Google</span>
        </Button>
      </div>
    </div>
  );
};

export default Index;
